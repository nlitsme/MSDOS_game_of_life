...
void gprintf(int x,int y, char *s, int a, int b, int c)
{
  char line[80];
  if (graphics)
  {
    sprintf(line,s,a,b,c);
...
int pass1(void)
{
  ...
  while (p!=NULL)
  {
    for (dx=-1 ; dx<=1 ; dx++)
      for (dy=-1 ; dy<=1 ; dy++)
      {
        if ((f=field[p->x+dx][p->y+dy])==NULL)
        {
          f=field[p->x+dx][p->y+dy]=(struct cell *)malloc(sizeof(struct cell));
          if (f==NULL)
          {
...
int readstart(char *inputfile)
{
  ...
  while (!error && fgets(line,XMAX,f)!=NULL)
  {
    ...
    if ((x||y) && !(len>0 && strchr(line,'*')==NULL) )
    {
      ...
      {
        p=strchr(line,'*');
        while (p!=NULL && x+p-line<XMAX && y<YMAX)
        {
          setxy(x+p-line+1,y+1);
          if ((cel=field[x+p-line][y])==NULL)
          {
            cel=(struct cell *)malloc(sizeof(struct cell));
            if (cel==NULL)
            {
              printf("Out of memory! aborting...\n");
              exit(1);
            }
            cel->x=x+p-line; cel->y=y;
            cel->nneigh=0;
          }
          cel->state=1;
          cel->n=oldlist;
